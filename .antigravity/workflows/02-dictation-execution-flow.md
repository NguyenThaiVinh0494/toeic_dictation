...
4. Kích hoạt Dịch nghĩa (Translation Reveal & Word Tooltip):

   - **Dịch toàn câu (Auto-Reveal):** Kiểm tra tỷ lệ chính xác (Accuracy). Nếu `Accuracy === 100%`, tự động thay đổi state `showTranslation` thành `true`. Sử dụng Framer Motion (`animate={{ height: "auto", opacity: 1 }}`) để xổ bản dịch tiếng Việt xuống ngay dưới câu tiếng Anh.
   - **Dịch từng từ (Click-to-Translate):** Khi render kết quả chuỗi Diffing, mỗi từ tiếng Anh (dù đúng hay sai) BẮT BUỘC phải được bọc trong một thẻ `<span>` có sự kiện `onClick`. Khi người dùng click vào một từ, kích hoạt Skill tra từ điển (xem `@04-word-translation-tooltip.md`) để hiển thị Popover giải nghĩa.
5. **Lưu Tiến độ (Persist Progress):**
